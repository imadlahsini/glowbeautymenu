...
export default function SalonMenu() {
  ...
  const calculateNodePosition = useCallback((index, total) => {
    ...
    const scale = 0.55 + depthFactor * 0.9; // 0.55 at back, 1.45 at front
      ...
      {!selectedCategory && (
        <div style={{
          ...
          <div
            ...
            style={{
              position: "relative",
              width: `${radius * 2 + nodeSize + 160}px`,
              maxWidth: "100%",
              height: `${radius * 2 + nodeSize + 160}px`,
              display: "flex", alignItems: "center", justifyContent: "center",
              cursor: "grab",
            }}
          >
            ...
            {(() => {
              ...
              const scaledSize = nodeSize * position.scale * (isFront ? 1.05 : 1);
              const borderWidth = isFront ? 3 : position.depthFactor > 0.5 ? 2 : 1.5;

              // Smooth radial offset — cubic curve pushes front out, back stays on ring
              const outwardOffset = Math.pow(position.depthFactor, 3) * 50;
              const dirX = position.x / radius;
              const dirY = position.y / radius;
              const finalX = position.x + dirX * outwardOffset;
              const finalY = position.y + dirY * outwardOffset;
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    transition: isDraggingRef.current ? "none" : "all 0.4s cubic-bezier(0.16, 1, 0.3, 1)",
                    cursor: "pointer",
                    padding: "16px", margin: "-16px",
                  }}
                >
                  {/* Spotlight glow — front node only */}
                  {isFront && (
                    <div style={{
                      position: "absolute",
                      ...
                      pointerEvents: "none",
                      transition: isDraggingRef.current ? "none" : "all 0.4s ease",
                    }} />
                  )}
                  ...
                  <div style={{
                    ...
                    color: "#fff",
                    opacity: isFront ? 1 : 0,
                    transition: isDraggingRef.current ? "none" : "opacity 0.3s ease",
